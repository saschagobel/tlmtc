...
RUN mkdir -p /dist \
    && uv export \
        ...
        --no-header \
        --no-annotate \
        --output-file /dist/requirements-train.txt \
...
LABEL org.opencontainers.image.title="tlmtc HPO GPU" \
      org.opencontainers.image.description="Single-process, single-GPU hyperparameter optimization for tlmtc" \
...
RUN python -m pip uninstall \
    ...
    && python -m pip install \
        ...
        --no-cache-dir \
        --require-hashes \
        --requirement /tmp/tlmtc-dist/requirements-train.txt \
    && python -m pip install \
        --break-system-packages \
        --no-cache-dir \
        --no-deps \
...
# Verify the GPU training stack and exclude ONNX dependencies.
RUN python - <<'PY'
...
for package in ("olive", "onnx", "onnxruntime"):
    assert importlib.util.find_spec(package) is None, f"Unexpected ONNX dependency: {package}"
